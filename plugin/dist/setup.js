...
import { defaultPluginRoot, discoverClaudeExecutables, readClaudeRuntimeConfig, writeClaudeRuntimeConfig, } from './claude-runtime.js';
...
function windowsStatusLineCommand(launcherPath, pluginRoot) {
...
export function buildStatusLineCommand(launcherPath, pluginRoot, platform = process.platform) {
    if (platform === 'win32')
        return windowsStatusLineCommand(launcherPath, pluginRoot);
    return `node ${commandArg(launcherPath)} ${commandArg(pluginRoot)}`;
}
export function configureInstallation(pluginRoot, executable) {
...
async function chooseClaudeExecutable(options, pluginRoot) {
...
if (invokedPath === comparablePath(fileURLToPath(import.meta.url)))
    void main();
