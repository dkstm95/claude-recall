...
- **디렉토리+브랜치 기반 accent color** — `cwd + 현재 브랜치` 조합의 결정적 색상 바. 텍스트 읽기 전에 색상으로 세션 구분. 세션 중 브랜치가 바뀌면 색도 바뀌어서 feature 브랜치가 base에서 시각적으로 분리됨.
...
- **line1** — 선택: `focus`, `branch`, `model`, `worktree`, `session`, `agent`, `pr`
...
| 요소 | 위치 | 설명 | 출처 |
|------|------|------|------|
| **accent bar** | 모든 줄, 왼쪽 | 결정적 색상 바 (`▍`) — `cwd + 현재 브랜치` 기반. 브랜치 변경 시 색도 변경 | claude-recall |
