...
| **elapsed** | 1줄, 오른쪽 | 마지막 활동 이후 경과 시간 | claude-recall |
| **model** | 1줄, 오른쪽 | 사용 중인 Claude 모델 (예: Opus 4.6) | Claude Code 빌트인 |
| **context%** | 1줄, 오른쪽 | 컨텍스트 윈도우 사용량 | Claude Code 빌트인 |
| **cost** | 1줄, 오른쪽 | 누적 세션 비용 | Claude Code 빌트인 |
| **turn** | 2줄, 왼쪽 | 현재 프롬프트 번호 (`#12`) | claude-recall |
| **last prompt** | 2줄 | 마지막으로 입력한 프롬프트 | claude-recall |

> [!TIP]
> 10회 이상 프롬프트를 입력하면 purpose 옆에 `(try /purpose)` 힌트가 나타납니다. `/purpose`를 실행하면 Claude가 대화 내용을 분석해서 더 정확한 목적을 제안합니다.
