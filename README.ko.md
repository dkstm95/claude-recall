...
<details>
<summary><strong>v5.x에서 업그레이드</strong></summary>

`/purpose` 커맨드가 제거되었습니다. Focus는 이제 자율 관리됩니다. 기존 `purpose` state 필드는 첫 읽기에서 자동으로 `focus`로 마이그레이션됩니다 — 사용자 조치 불필요. 전체 Breaking 변경사항은 [CHANGELOG.md](CHANGELOG.md) 참조.
