...
export function getClaudeRuntimeConfigPath() {
    return join(getRecallDir(), 'runtime.json');
}
function isRecord(value) {
    return !!value && typeof value === 'object' && !Array.isArray(value);
}
export function readClaudeRuntimeConfig() {
    const value = readJsonFile(getClaudeRuntimeConfigPath());
    if (!isRecord(value))
        return null;
...
export function writeClaudeRuntimeConfig(executable) {
...
function binaryMagic(path) {
    const fd = openSync(path, 'r');
    try {
        const bytes = Buffer.alloc(4);
        if (readSync(fd, bytes, 0, bytes.length, 0) < 2)
            return false;
 ...
 */
export function isUsableClaudeExecutable(path, platform = process.platform) {
    if (!isAbsolute(path))
        return false;
...
export function resolvePinnedClaudeExecutable() {
    const config = readClaudeRuntimeConfig();
    if (!config || !isUsableClaudeExecutable(config.claudeExecutable))
        return null;
    return config.claudeExecutable;
}
export async function resolveVerifiedPinnedClaudeExecutable(verify = verifyClaudeExecutable) {
    const pinnedPath = resolvePinnedClaudeExecutable();
    if (!pinnedPath)
        return null;
    let targetSnapshot;
    try {
        targetSnapshot = realpathSync(pinnedPath);
    }
    catch {
        return null;
    }
    if (!isUsableClaudeExecutable(targetSnapshot))
        return null;
...
function candidateKey(path, platform) {
    const normalized = resolve(path);
    return platform === 'win32' ? normalized.toLowerCase() : normalized;
}
function nativeDefaultPath(options) {
...
function candidatePaths(options) {
    ...
    if (options.existingPath)
        paths.push(options.existingPath);
...
function isForbiddenAutoCandidate(path, options) {
    ...
    catch { /* usability check reports it */ }
    return roots.some((root) => {
        let realRoot = root;
        try {
            realRoot = realpathSync(root);
        }
        catch { /* lexical root remains useful */ }
        return [root, realRoot].some((base) => pathContains(base, path) || pathContains(base, target));
    });
}
export function verifyClaudeExecutable(path) {
    return new Promise((resolveVersion) => {
        ...
        child.stdout.on('data', (chunk) => { stdout = (stdout + chunk).slice(0, 1_000); });
...
export async function discoverClaudeExecutables(options = {}) {
    const platform = options.platform ?? process.platform;
    const verified = [];
    for (const path of candidatePaths(options)) {
        if (isForbiddenAutoCandidate(path, options) || !isUsableClaudeExecutable(path, platform))
            continue;
        const version = await (options.verifyVersion ?? verifyClaudeExecutable)(path);
        if (version)
            verified.push({ path: resolve(path), version });
    }
    return verified;
}
