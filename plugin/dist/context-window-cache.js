...
export async function resolveContextWindow(sessionId, live) {
    ...
    if (livePct === undefined)
        return { used_percentage: cachedPct ?? 0 };
...
export async function cleanupContextCache(keptSessionIds) {
    if (!Object.keys(readCache()).some((id) => !keptSessionIds.has(id)))
        return;
    try {
