...
> [!IMPORTANT]
> `/claude-recall:setup` 후 **Claude Code를 재시작**해야 statusline과 새 훅이 활성화됩니다.
...
> [!NOTE]
> **6.4.2 이하에서 업그레이드하는 경우:** `/reload-plugins` 실행 후 `/claude-recall:setup`을 실행하고 Claude Code를 한 번 재시작하세요. setup은 검증된 Claude Code stable launcher의 절대 경로를 private recall 디렉터리에 고정하며, 백그라운드 focus 갱신은 의도적으로 런타임 PATH fallback을 사용하지 않습니다. 이 업데이트는 설치 캐시도 경량 runtime 전용 번들로 전환합니다. 이후 릴리스에서 별도 안내하지 않는 한 일반 업데이트에는 setup을 다시 실행할 필요가 없습니다.
...
- **line1** — 선택: `focus`, `branch`, `model`, `worktree`, `session`, `agent`, `pr`. 우측 슬롯 우선순위는 배열 순서를 따르며, 폭이 줄면 뒤쪽 항목부터 생략됩니다.
...
| **elapsed** | 2줄, 오른쪽 | 세션 시작 후 wall-clock 경과 시간 | claude-recall |
...
| **worktree** *(옵션)* | 1줄, 오른쪽 | 링크드 git worktree 안에서 Claude Code의 `worktree.name` / `worktree.path` 기반 `⎇ <이름>` 표시 | Claude Code 빌트인 |
...
| **pr** *(옵션)* | 1줄, 오른쪽 | Claude Code의 `pr` 필드 기반 활성 PR 번호/제목 | Claude Code 빌트인 |
...
- **첫 진입 캐시.** Claude Code는 첫 API 호출 전에는 `rate_limits`와 `context_window`를 stdin에 싣지 않기 때문에, claude-recall은 마지막에 본 값을 Claude 설정 디렉터리(기본 `~/.claude/claude-recall/`) 아래에 캐싱해두고 첫 렌더링에서 복원합니다 — 첫 프롬프트를 기다릴 필요 없이 바가 바로 나타납니다. 자세한 동작은 CHANGELOG v6.1.4 / v6.1.5 참고.
...
각 트리거는 하나의 갱신 lease를 획득한 뒤 setup에서 고정한 Claude Code launcher를 `-p --model=haiku`로 spawn합니다. compact summary가 있으면 이를 사용하고, 없으면 마지막 12KB transcript를 사용합니다:
