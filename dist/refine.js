...
import { classifyError, spawnRefinement } from './refine-spawn.js';
import { launchRefinementWorker } from './refine-worker-launch.js';
const DEBOUNCE_MS = 5_000;
export { classifyError, isRefiningSubprocess, launchRefinementWorker, readTranscriptTail, REFINING_ENV_VALUE, REFINING_ENV_VAR, spawnRefinement, };
...
function recordRefinementResult(sessionId, result) {
...
export async function triggerFocusRefinement(sessionId, transcriptPath, preferredTranscript) {
    ...
    if (!shouldRefine(state.lastRefinedAt))
        return;
    const transcript = await resolveRefinementTranscript(transcriptPath, preferredTranscript, state.lastUserPrompt);
