...
<p align="center">
  <img src="https://img.shields.io/badge/version-6.1.5-blue?style=flat-square" alt="version">
...
| **컨텍스트 경고** | 1줄, 오른쪽 | 70-89%일 때 흐린 `(try /handoff)`, 90%↑일 때 빨간 `⚠ try /handoff` — 3줄을 꺼도 보장 | claude-recall |
...
- **첫 진입 캐시.** 세션 진입 직후에는 Claude Code가 아직 API 호출을 하지 않아 stdin에 `rate_limits`와 `context_window`가 비어 있습니다. claude-recall은 마지막에 본 값을 `~/.claude/claude-recall/rate-limits.json`(계정 단위) 및 `~/.claude/claude-recall/context-windows.json`(세션 단위)에 저장해두고 첫 렌더링에서 이 캐시로 5h / 7d / ctx 바를 즉시 표시합니다. rate-limit 창은 `resets_at`이 지나면 캐시에서 제외되고(새 창 사용량 과대 표시 방지), context 캐시는 세션이 7일 idle로 만료될 때 함께 정리됩니다.
- 좁은 터미널에서는 3번째 줄의 `cost` → `7d` → `5h` 순으로 생략되어 `ctx` 바가 마지막까지 남습니다 — 컨텍스트 고갈이 가장 시급한 신호이기 때문입니다.
- 컨텍스트 **70-89%**에선 1줄에 흐린 `(try /handoff)` 힌트, **90% 이상**에선 빨간 `⚠ try /handoff` 경고가 나타납니다. 경고는 Line 1에 있으므로 `line3: []`로 3줄을 완전히 꺼도 항상 보입니다.
