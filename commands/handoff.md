...
2. Read the session state JSON. Use only these fields: `sessionId`, `focus`, `branch`, `cwd`, `promptCount`, `lastUserPrompt`, `lastActivityAt`. Do NOT reference any other field names.
...
4. Generate a filename slug:
   - If `focus` is non-empty: lowercase it, replace whitespace with `-`, strip any character not in `[a-z0-9\-가-힣ㄱ-ㅎ]`, and truncate to 30 characters.
   - If the slug is empty after this (or `focus` was empty): use `sessionId.slice(0, 8)` as the slug.
...
```markdown
# Session handoff from: {focus or "(no focus)"}
