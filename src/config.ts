...
export interface HudConfig {
  line1: string[];  // e.g. ['purpose', 'branch', 'model']
  line2: string[];  // e.g. ['turn', 'prompt', 'elapsed', 'context', 'cost']
...
export function readConfig(): HudConfig {
  try {
    ...
    return {
      line1: Array.isArray(parsed.line1) ? parsed.line1.filter((s: unknown) => VALID_LINE1.includes(s as string)) : DEFAULT_CONFIG.line1,
      line2: Array.isArray(parsed.line2) ? parsed.line2.filter((s: unknown) => VALID_LINE2.includes(s as string)) : DEFAULT_CONFIG.line2,
